...
export class Favorites {
    ...
    async add(username) {
        try {
            const isExist = this.entries.find(entry => entry.login === username)
...
export class FavoritesView extends Favorites {
    ...
    createRow() {
        ...
        tr.innerHTML = `<td class="user">
            <img src="https://github.com/vitorianobre.png" alt="Foto de Vitoria Nobre">
            <a href="https://github.com/vitorianobre" target="_blank">
                <p class="name">Vitoria Nobre</p>
                <span class="username">/vitorianobre</span>
            </a>
        </td>
    ...
    emptyView() {
        const isEmpty = (this.root.querySelectorAll('tbody tr').length == 0) ? true : false;

        if (isEmpty) {
            const rowEmpty = this.createRow()
            rowEmpty.innerHTML = `<td colspan = "4" class="empty" >
